...
A special thanks goes to all those that edited the book via GitHub pull requests or made suggestions by creating an _issue_: Amy Gill (`gillsignals`), Huang Qiang (`nickyfoto`), Marc-André Désautels (`desautm`), Michail Schwab (`michaschwab`), Alvaro Larreategui (`alvarolarreategui`), Jake VanCampen (`jakevc`), Guillermo Lengemann (`omerta`),  Enrico Spinielli (`espinielli`), Aaron Simumba (`asimumba`) and Matthew Ploenzke (`mPloenzke`).
