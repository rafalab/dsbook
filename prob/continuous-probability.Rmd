...
However, the approximation is not as useful for other intervals. For instance, note how the approximation breaks when we try to estimate:
...
\mbox{Pr}\left(Z \leq \frac{a-\mu}{\sigma} \right)
    ...
    So, no matter the units, the probability of $X\leq a$ is the same as the probability of a standard normal variable being less than $(a - \mu)/\sigma$. If `mu` is the average and `sigma` the standard error, which of the following R code would therefore give us the right answer in every situation:
