...
With the right argument, `ggplot` automatically shades the intersecting region with a different color. We will show examples of __ggplot2__ code for densities in Section \@ref(gapminder).
...
The pre-built functions `mean` and `sd` (note that for reasons explained in Section \@ref{data-driven-model}, `sd` divides by `length(x)-1` not `length(x)`) can be used here:
```{r}
m <- mean(x)
s <- sd(x)
c(average = m, sd = s)
