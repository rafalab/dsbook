# Case study: Trends in world health and economics {#gapminder}
...
## Stratify and visualize distributions

A histogram showed us that the income distribution values show a dichotomy. However, the histogram does not show us if the two groups of countries are _west_ versus the _developing_ world.  
...
Note that in the plot we just made the regions are ordered alphabetically. But there is no practical reason for this to be the case. Data exploration is often made easier if we order the strata by some interpretable quantity. This can be achieved using the `reorder` function which we covered in Section \@ref(#functions-we-use-a-lot).
...
Note that we have to invert the `x` and `y` used for the boxplot. A useful `geom_density_ridges` is `scale` which let's you determine the amount of overlap, with `scale = 1` meaning no overlap and larger values resulting in more overlap. 
...
By default, the height of the points is jittered and should not be interpreted in anyway. To show data points, but without using jitter we can use the following code to add what is referred to as a _rug representation_ of the data.  
...
Data exploration clearly showed that in 1970 the was a "west versus the rest" dichotomy. But does this dichotomy persist? Let's use `facet_grid` see how the distributions have changed. To start, we will focus on two groups: the west and the rest. We make four histograms.
...
`r round(gapminder %>% filter(year==present_year) %>% summarize(perc=sum(population[country%in%country_list], na.rm=TRUE)/sum(population, na.rm=TRUE)) %>% .$perc*100 )`% of the world population, so this subset should be representative.
...
We now see that while the rich countries have become a bit richer, but percentage-wise, the poor countries appear to have improved more. In particular, we see that the proportion of _developing_ countries earning more than $16 a day increases substantially. 
...
The previous data exploration suggest that the income gap between rich and poor countries has narrowed considerably during the last 40 years. 
We used a series of histograms and boxplots to see this. Now we suggest a succinct way to convey this message with just one plot. 
...
Another way to show this ahieved is by stacking the densities on top of each other:
