...
In many cases, we do not show the data because it adds clutter to the plot and obfuscates the message. In the example above, there are not many points so adding them actually lets us see all the data. We can add this layer using geom_point():
...
We can now easily plot the densities for each region. We use `color` and `size` to clearly see the tops:
