...
__geometry__ component. Other possible geometries are barplot, histogram, smooth densities, qqplot, and boxplot. 
...
knitr::include_graphics(c("dataviz/img/ggplot2-cheatsheeta.png",
                          "dataviz/img/ggplot2-cheatsheetb.png"))
```
...
Now becuase the points are larger, it is hard to see the labels. If we read the help file for `geom_text`, we see the `nudge_x` argument, which moves the text slightly to the right or to the left.
...
## Other geometries: histograms, density, and boxplot  {#other-geometries}
...
If we have values in a vector, say:
