...
Just joining these two tables together will not work since the order of the states is not the same.
...
Suppose we want a table like `tab_1`, but adding electoral votes to whatever states we have available. For this, we use left join with `tab_1` as the first argument. We specify which column to use to match with the `by` argument.
...
The function __dplyr__ includes an `intersect` function that can be applied to tables with the same column names. This function returns the rows in common between two tables. To make sure we use the __dplyr__ version of interesect rather than the base package version we can use `dplyr::intersect` like this:
...
The set difference between a first and second argument can be obtained with `setdiff`. Unlike `instersect` and `union`, this function is not symmetric:
