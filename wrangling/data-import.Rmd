...
When creating spreadsheets with text files like the ones created with a simple text editor, a new row is defined with return and columns with some predefined special character [AN: "and columns are defined by/with some...?]. The most common characters are comma (`,`), semicolon (`;`), white space (\  ) and tab (\ \ \ \ ). Here is an example of what a comma separated file looks like if we open it with a basic text editor:
