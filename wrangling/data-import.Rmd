...
In the R chapter, we covered some of the basics of importing data. We described functions available in the default R installation. Here, we present a more general discussion and introduce the __tidyverse__ packages __readr__and __readxl__.
...
Note that we receive a message letting us know what data types were used for each column. Also note that `dat` is a `tibble` not just a data frame. This is because `read_csv` is a __tidyverse__ parser. We can see that the data has in fact been read-in with the content in the file:
