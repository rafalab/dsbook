# String Processing  stringr and lubridate
...
One of the most common data wrangling challenges involves converting or extracting numeric data, contained in character strings, into the numeric representations required to make plots, summarize or fit models in R. Also common is processing unorganized text into meaningful variable names or categorical variables. Extracting dates and times are particularly challenging becuase in principle they represent numbers but are expressed as strings following different conventions.
...
Base R includes functions to perform all these tasks. However, they don't follow a unifying convention which makes it a bit hard to memorize and use. The __stringr__ package basically repackages this functionality, but uses a more consistent approach of naming functions and ordering their arguments. For example, in __stringr__, all the string processing functions start with `str_`. This means that if you type `str_` and hit tab, R will auto-complete and show all the available functions. As a result, we don't necessarily have to memorize all the function names. Another advantage is that the string is always the first argument, which means we can more easily use the pipe. We therefore will be focusing on the __stringr__ package. However, because the R-base equivalents are so widely used, below we include a table describing the __stringr__ functions and include R-base equivalent when available.
...
If we print out `s` we see that the doulbe quotes are _escaped_ with the backslash `\`.
```{r}
s
```
...
In this section we will use the following datasets as an example:
...
The table below includes the functions available to you in the __stringr__ package. We split them by task. We also include the R-base equivalent when available. 
...
To specify patterns that we do **not** want to detect, we can use the `^` symbol but only __inside__ square brackets. For example, if we want to detect digits that are preceeded by anything except a letter we can do the following:
...
However, we still have many cases to go. Note that in the code above, we leveraged the __stringr__ consistency and used the pipe. 
...
## Case study 2: Reported heights (continued)
...
The `extract` function from the __tidyr__ package lets us use regex groups to extract the desired values. Above we showed how to use `separate` to split the data as desired. Here is the equivalent code using `extract`:  
```{r}
...
If you learn more about the __purrr__ package, you will be able to write the code above more efficiently:
