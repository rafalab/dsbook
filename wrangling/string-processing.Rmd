...
Escaping characters is something we often have to do when processing strings.
...
| Locate | Show all the parts of the string that match the pattern. | `str_view_all` | 
...
Above we used code to scrape the web and create the `murders_raw` table. We noted that three columns need to be parsed from characters into numbers, but that commas were making it hard. We can use the `str_detect` function to see that two of the three columns have commas in the entries:
...
The pattern supplied to the __stringr__ functions can be a regex rather than a standard string. We will learn how this works through a series of examples.


### A string

Technically any string is a regex, perhaps the simplest example is a single character. The comma `,` used here:
...
_Groups_ are a powerful aspect of regex that permits the extraction values. Groups are defined using parentheses. They don't affect the pattern matching per-se. Instead, groups permit tools to identify specific parts of the pattern so we can extract them. 
...
we see that it is a long string and each line on the page, including the table rows, is separated by the symbol for newline: `\n`. We therefore can create a list with the lines of the text as elements as follows:
...
18. Now that we have wrangled this data step-by-step, put it all together in one R chunk, using the pipe as much as possible. Hint: first define the indices, then write one line of code that does all the string processing.
