...
For the inches part we can have one or two digits. This can be specified in regex with _quantifiers_. This is done by following the pattern by curly brackets containing the number of times the previous entry can be repeated. We use an example to illistrate: the pattern for one or two digits is:
...
It turns that we can avoid all the work shown above after the call to `str_split`. Specifically, if we know that the data we are extracting can be reprented as a table, we can use the argument `simplify=TRUE` and `str_split` return a matrix instead of a list:
...
Now we will look that the second line: 
```{r}
the_names_2
```
