...
Although we do not describe the details, know that in R we can use the `glm` function to estimate the log odds ratio and its standard error. The general idea is to perform a type of regression in which compute the effect of gender on the counts. We have to form a matrix with success in the first row and failures in the second row which we can form like this:

```{r}
y <- rbind(two_by_two$men[2:1], two_by_two$women[2:1])
y
```

Then form a covariate representing gender:

```{r}
men <- c(TRUE, FALSE) 
```

Now we the function `glm` function to estimate the log odds

```{r}
fit <- glm(y ~ men, family = "binomial")
summary(fit)$coef
```

Note that we get the same answer as above:
...
But we can now construct confidence intervals for the odds ratios:
