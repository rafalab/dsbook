...
As we described earlier, a few weeks before the 2012 election Nate Silver was giving a Obama a 90% chance of winning. How was Mr. Silver so confident? We will use a Monte Carlo simulation to illustrate the insight Mr. Silver ha and others missed. To do this, we generate results for 12 polls taken the week before the election. We mimic sample sizes from actual polls and construct and report 95\% confidence intervals for each of the 12 polls. We save the results from this simulation in a data frame and add a poll ID column.
...
avg_poll <- data.frame(poll = "Avg", estimate = d_hat, low = d_hat-moe, high = d_hat+moe, sample_size = sum(polls$sample_size))
polls2 <- rbind(polls,avg_poll)
