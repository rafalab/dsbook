...
In this chapter we will demonstrate how _poll aggregators_, such as FiveThirtyEight, collected and combined data reported by different experts to produce improved predictions. Statistical models, the main statistical tool used by the aggregators are the topic of this chapter. We build on the statistical inference concepts we learned in Chapter \@ref(inference). We start with relatively simple models, realizing that the actual data science exercise of forecasting elections involves rather complex ones, which we introduce later in Section \@ref(election-forecasting).
...
In this section we will introduce ideas behind the _statistical models_, also known as _probability models_, that were used by Fivethirtyeight, and other poll aggregators, to improve the election forecasts power of individual polls. In this chapter we motivate the models. These models are written out mathematically in Section \@ref(election-forecasting).
...
new_row <- data_frame(d_hat, d_hat-moe, d_hat+moe, sum(polls$sample_size),13)
...
We introduce actual data from the 2016 US presidential election to show how models are motivated and built to produce these predictions. To understand the "81.4% chance" statement we need to describe Bayes Statistics, which we do in Sections \@ref(bayesian-statistics) and \@ref(bayesian-approach). 
