...
6\. Now let's repeat the above for each year since 1962 and make a plot. Use `summarize` and the __broom__ package to fit this model for every year since 1962. 

   
7\. Use the results of the previous exercise to plot the estimated effects of BB on runs.

   
8\. __Advanced__. Write a function that takes R, HR, and BB as arguments and fits two linear models: `R ~ BB` and `R~BB+HR`. Then use the `summary` function to obtain the `BB` for both models for each year since 1962. Then plot these against each other as a function of time.
