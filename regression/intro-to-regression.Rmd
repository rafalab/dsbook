...
For example, the correlation between a variable and itself is 1, and the correlation between a variable and its negative is -1.  For other pairs, the correlation is in between -1 and 1. The correlation between father and son's heights is about 0.5:
...
Note that a 72 inch father is taller than average. Specifically, that is (72 -  `mean(galton_heights$father)`/`sd(galton_heights$father)` =
...
A more technical way to define the bivariate normal distribution is the following: if $X$ and $Y$ are each normally distributed random variables, and for any stratum of $X$ ($X=x$), $Y$ is approximately normal in that stratum, then the pair $(X,Y)$ is approximately bivariate normal. 
...
$$ f_{Y \mid X=x} \mbox{ is the notation for conditional distribution and}$$

$$\mbox{E}(Y \mid X=x) \mbox{ is the notation for conditional expected value.}$$
...
We get $\mbox{E}(X \mid Y=y) =$ `r b_2` + `r m_2`y. Again we see regression to the average: the prediction for the father is closer to the father average than the son heights $y$ is to the son average.
...
1. Load the `GaltonFamilies` data from the __HistData__. The children in each family are listed by gender and then by height. Create a dataset called `galton_heights` by picking a male and female at random. Set the seed to 1 before running this code as it involves a random sample. 
