...
Up to this point, this book has focused mainly on _univariate_ variables. However, in data science applications, it is very common to be interested in the relationship between two or more variables. For instance, in Chapter \@ref(linear-models) we will use a data driven approach, that examines the relationship between player statistics and success, to guide the building of a baseball team with a limited budget.  But before delving into this more complex example, we introduce necessary concepts needed to understand regression using a simpler illustration. We actually use the dataset from which regression was born.
