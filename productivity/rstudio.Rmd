...
We now need to pay attention to the Git pane. It is important to know that **your local files and the GitHub repo will not be synced automatically**.  You have to sync when you are ready and we show you how below. To truly understand why RStudio does not automatically sync, we need to learn more details about Git, and we will do so in Section \@ref(git). Right now, we will quickly show you how to sync with this simple example. 
