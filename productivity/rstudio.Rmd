...
We now need to pay attention to the Git pane. It is important to know that **your local files and the GitHub repo will not be synched automatically**.  You have to synch when you are ready and we show you how below. To truly understand why RStudio does not automatically synch we need to learn more details about Git, and we will do so a bit later. Right now, we will quickly show you how to sync with this simple example. 
...
Note that in the pop-up window we no longer see the `code.R` file. This is because it no new changes have been performed since we last pushed. We can exit this pop-up window now and continue working on our code.
