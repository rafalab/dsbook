...
Eventually we want these new version of the files to be tracked and 
synched with the upstream repo. But we don't want to keep track of every little change: we don't want to synch until we are sure these are final enough versions to share. For this reasons edits in the staging area are not kept by the version control system.
...
Notice what that status says now:
...
Because `new-file.txt` is stged, the current version of the file will get added to the local repository next time we commit. We commit like this:
...
The next step is to connect the local repo with the GitHub repo. In a previous example, we had RStudio do this for us. Now we need to do it ourselves. We can adding any of the files and committing it: 
