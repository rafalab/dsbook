...
But how does it all get started? There are two ways. We can clone an existing repo or initialize one. We will try the cloning first.
...
Because `new-file.txt` is staged, the current version of the file will get added to the local repository next time we commit. We commit like this:
