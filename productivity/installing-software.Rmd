...
4. Git Bash (for Windows users only) - A piece of software that emulate Unix on Windows machines.
...
(CRAN). Search for CRAN on your browser:
...
3. Once at the CRAN download page you will have several choices. You want to install the _base_ subdirectory. This installs the basic packages you need to get going. We will later learn how to install other needed packages from within R, rather than this webpage.
...
```
    ...
    When selecting the langunage consider that it will be easier to follow this book if you select English.

    ```{r}
knitr::include_graphics(screenshots[8]) 
```

    Continue to selectall the defaults:
...
If you followed the default installation, on Windows a shortcut appears on your desktop which you can click to start R.
...
But we will be much more productive using an editor developed for coding, such a the one provided by RStudio. In the next section we demonstrate how to install RStudio.
...
5. Once the installation file is download, click on the downloaded file to start the installation process:
...
Congratulations! You have installed RStudio. You can now started as you start any other program in your computer. 
...
Git is most effectively used using Unix, although one can also use it through RStudio. In the next section we describe Unix in more detail. Here we show you how to install software that permits you to use Git and Unix. The installation process is quite different for Windows and Mac, so we include two different sections.
...
4. In one of the installation steps you will be asked to pick the default editor for Git. Unless you are already a _vi_ or _vim_ user, here we recommend against selecting _vim_ which might be the default. If you do not recognize an editor you know among the options given, we recommend that you select _nano_ as your default editor for Git since it is the easiest to learn:
...
9. and it should look something like this:
...
**Reminder**: On Windows we install Git Bash. We not need to do this on the Mac since they come with the terminal pre-installed and we can use this to run Unix commands.
