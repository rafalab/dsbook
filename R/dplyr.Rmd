...
Up to now we have been changing vectors by reordering them and subsetting them through indexing. However, once we start more advanced analyses, we will want to prepare data tables for data analysis. We refer to this task as data wrangling. For this purpose we introduce the __dplyr__ package, which provides intuitive functionality for working with tables. We will explore this topic in much more depth in the Data Wrangling part of the book. 
...
__dplyr__ is also included in the __tidyverse__ package, which bundles together several packages for data science that share similar function calls, behavior and design pinciples. We will learn several other __tidyverse__ packages throughout the course. If you load the __tidyverse__ package, that will load __dplyr__ among other packages. You can load __dplyr__ through the __tidyverse__ like this:
