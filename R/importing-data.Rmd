...
Now that we can copy a files into our home directory using the code below, we are ready to read-in files. 
...
With experience you will learn how to deal with different challenges. Carefully reading the help files for the functions discussed here will be useful. Two other functions that are helpful are `scan` and `read_lines`. With scan you can read-in each cell of a file. Here is an example:
...
12. __Save the Data as Text Files__ - Save files for sharing in comma or tab delimited format.
