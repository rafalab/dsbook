...
```{r, echo=FALSE}
knitr::include_graphics("R/img/csv-file.png")
```
...
A spreadsheet containing the US murders data is included as part of the __dslabs__ package. Finding this file is not straight forward, but the following lines of code copy the file to the folder in which R looks in by default. We explain how these lines work below.
...
This code does not read the data into R, it just copies a file. But once the file is copied, we can import the data with a simple line of code. Here we use the `read_csv` function from the __readr__ package.
...
You can think of your computer's filesystem as a series of nested folders, each containing other folders and files. We refer to the folder that contains all other folders as the The _root directory_. Data scientists refer to folders as _directories_. We refer to the directory in which we are currently located as the _working directory_. The working directory therefore changes as you move through folders: think of it as your current location. 
...
These relative paths give us the location of the files or directories if we start in the directory with the full path `r dir`. 

**Note**: You will probably not make much use of the `system,file` function in your day-to-day data anlysis work. We introduce it in this section becuase it facilitates the sharing of spreadsheets by including them in the __dslabs__ package. You will rarerly have the luxury of data being included in pacakges you already have installed. 
...
Now that we can copy a files into our home directory using the code
below we are ready to read-in files. 
...
The __readr__ library includes functions for reading data stored in text file spreadsheets into R. 
...
In our experience this can be a cause for confusion since a variable that was saved as characters in file is converted to factors regardless of what the variable represents. In fact, we **highly** recommend setting `stringsAsFactors=FALSE` to be your default approach when using the R-base parsers.
