...
For most functions, we can also use the shorthand:
...
We've used the letters _a_, _b_, and _c_ as variable names, but variable names can be almost anything. Some basic rules in R is that they have to start with a letter, can't contain spaces and should be variables that are predefined in R. For example, don't use `install.packages` as a variable name and don't type 
`install.packages <- 2`!
...
Values remain in the workspace until you end your session or erase them with the function `rm`. But workspaces can be saved for later use. In fact, when you quit R, the programs asks you if you want to save your workspace. If you do save it, the next time you start R, the program will restore the workspace. 
