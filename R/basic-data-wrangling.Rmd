...
This is one of the main features of dplyr. Functions in this package, such as `mutate`, know to look for variables in the data frame provided in the first argument. In the call to mutate above, `total` will have the values in `murders$total`. This approach makes the code much more readable. 
...
6. We can also use the `%in%` to filter with `dplyr`. You can therefore see the data from New York and Texas like this:
