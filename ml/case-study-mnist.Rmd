...
```{r mnist-rf, message=FALSE, warning=FALSE}
library(Rborist)
...
And here are the errors for the random forest:
...
1. Use the training set to build a model with several of the models available from the __caret__ package. For example, you can try these:
    ...
    We have not explained many of these but apply them anyway using `train` with all the default parameters. Keep the results ina a list. You might need to install some packages. Keep in mind that you will likely get some warnings.
