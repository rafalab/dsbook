...
library(Rborist)
control <- trainControl(method="cv", number = 10, p = .9)
grid <- expand.grid(predFixed = c(5, 10, 15, 25), 
                    minNode = c(1, 3))
train_rf <-  train(x[, col_index], y, 
                   method = 'Rborist',
                   nTree = 150,
...
fit_rf <- Rborist(x[, col_index], y,  minNode = 3, predFixed = 12, nTree = 100)
fit_rf <- randomForest::randomForest(x[, col_index], y, 
                       minNode = 3, mtry = 12)
...
```{r}
y_hat_rf <- predict(fit_rf, x_test[ ,col_index])$yPred
