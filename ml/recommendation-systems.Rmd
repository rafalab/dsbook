...
Finally we use `pivto_wider` to make a matrix with users represented by rows and movies by the columns
...
Statistics textbooks refer to the $b$s as effects. However, in the Netflix challenge papers, they refer to them as _bias_, thus the $b$ notation.
...
with the number of ratings this rating was based on.
...
These all seem like obscure movies. Do we really think these are the top 3 movies in our database? Will this prediction hold on the test set? Note that all lower in the test, some considerable lower:
...
Note that these estimates are based on a small number of ratings. The supposed _best_  movies were rated by very few users. Note that the small sample sizes lead to uncertainty. Therefore, larger estimates of $b_i$, negative or positive, are more likely. 
...
```{r regularization-shrinkage, fig.asp=1, echo=FALSE}
...
those that like gangster movies and dislike romance movies (coded as 2), those that like romance movies and dislike gangster movies (coded as -2), and those that don't care (coded as 0). The main point here is that we can almost reconstruct $r$, which has 60 values, with a couple of vectors totaling 17 values. Note that `p` and `q` are equivalent to the patterns and weights we described in Section \@ref(pca).
...
Because for the techniques shown here we can't have missing values we need to replace the missing ratings. There are advanced techniques for doing this, some are exaplined in the description of the winning entry for the Netflix competition. Here we will use a simple approach: replace with a constant. Now because an unrated movie is more likely to be a movie the user does not like, we will replace the missing ratings with -1 rather than a 0, which represents a neutral rating.
...
Just by looking at the top 10 in each direction, we see a meaningful pattern. The first PC shows the difference between Hollywood blockbusters on one side:
