...
We described how methods such as LDA and QDA are not meant to be used with many predictors $p$ because the number of parameters that we need to estimate becomes too large. [AN: review following sent] For example, with the digits example $p=784$, so QDA would have 784 parameters we would have over 600,000 parameters with LDA and we would multiply that by the number of classes for QDA. Kernel methods such as kNN or local regression do not have model parameters to estimate, but they also face a challenge when multiple predictors are used due to what is referred to the _curse of dimensionality_. The _dimension_ here refers to the fact that when we have $p$ predictors, the distance between two observations is computed in $p$-dimensional space.
...
Let's very quickly see how we can do [AN: "do' what?] using kNN:
...
Specifically, we define the following decision rule. If eicosenoic is larger than 0.065, predict Southern Italy. If not, then if linolenic is larger than $10.535$, predict Sardinia and Northern Italy otherwise [AN: remove "otherwise'?]. We can draw this decision tree like this:
...
[AN: review following sent] The first is that rather than taking the average, in each partition we predict with class that forms the observations in the training set. 
...
[AN: review following sent] Here is the Random Forest fit for our digits example two predictor:
