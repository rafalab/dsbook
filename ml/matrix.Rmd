...
Machine learning problems often involve datasets that large or larger than the MNIST dataset. There are variety of computational techniques and statistical concepts that are useful for the analysis and large dataset. In this chapter we scratch the surface of these techniques and concepts by describing matrix algebra, dimension reduction, regularization and matrix factorization. We use recommendation systems related to movie ratings as motivating example.
...
entries. In matrix algebra, we use the following notation for a vector representing a feature/predictor:
...
The _dimension_ of a matrix is often an important characteristic needed to assure that certain operations can be performed. The _dimension_ is a two number summary defined as the number of rows $\times$ the number of columns. In R, we can extract the dimension of a matrix with the function `dim`:
...
We will now learn several useful operations related to matrix algebra. We use three of the motivating examples listed above.
...
```{r flip-matrix-image, fig.width = 4, fig.height = 4}
...
```{r pixel-variance, fig.width = 4, fig.height = 4}
...
In Section \@ref(matrices) Section, we described the operations used to extract columns: 
...
We notice a clear dichotomy which is explained as parts of the image with ink and parts without. If we think that values below, say, 25 are smudges, we can quickly make them zero using:
...
```{r binarized-image, echo=FALSE, fig.width = 7, fig.height = 3.5}
rafalib::mypar(1,2)
rows <- 1:28
columns <- 1:28
...
In R, if we subtract a vector from a matrix, the first element of the vector is subtracted from the first row, the second element from the second row, and so on. So using mathematical notation, we would write it as follows:
...
4. Add the scalar 1 to column 1, the scalar 2 to column 2, and son on, to the matrix `x`. Hint: use `sweep` with `FUN = "+"`.
