...
If we want to change of how we perform cross validation, we can use the `trainControl` function. We can make the code above go a bit faster by using, for example, 10-fold cross validation. This means we have 10 samples using 10% of the observations each. We accomplish this:
...
[Here](https://topepo.github.io/caret/train-models-by-tag.html), we see that we need to install the `gam` package if we have done so already:
