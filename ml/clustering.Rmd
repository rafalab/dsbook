...
row_names <- str_trunc(x$title, 20)
x <- x[,-1] %>% as.matrix()
...
```{r dendrogram,fig.width=10.5,fig.height=5.25}
plot(h)
```
...
```{r dendrogram-2,fig.width=10.5,fig.height=5.25}
plot(h_2)
```
...
Before using k-means, we will fill in the NA with 0s.

```{r}
x[is.na(x)] <- 0
k <- kmeans(x, centers = 10)
```
...
```{r}
k <- kmeans(x, centers = 10, nstart = 25)
groups <- k$cluster
split(names(groups), groups)
```
