...
```{r qda-estimate, echo=FALSE, out.width="100%"}
p1 <- plot_cond_prob() + ggtitle("True conditional probability")

p2 <- plot_cond_prob(predict(train_qda, newdata = mnist_27$true_p)[,2]) +
  ggtitle("QDA")
...
p2 <- plot_cond_prob(predict(train_lda, newdata = mnist_27$true_p)[,2]) +
  ggtitle("LDA")

grid.arrange(p2, p1, nrow=1)
```

In the case of LDA, the lack of flexibility does not permit us to capture the nonlinearity in the true conditional probability function. 
...
1. Create a dataset with just the cerebella and hippocampi, two parts of the brain, and a predictor matrix with 10 randomly selected columns.
