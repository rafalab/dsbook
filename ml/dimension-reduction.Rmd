...
A scatter-plot quickly reveals that the correlation is high and that there are two groups of twins, the adults (upper right points) and the childer (lower left points):
...
```{r one-dim-approx-to-dist, echo = FALSE, fig.width=4, fig.height=4}
plot(dist(x), dist(z))
abline(0,1, col = "red")
```
...
then the underestimation goes away. We divde the distance by $\sqrt{2}$ to achieve the correction.

```{r distance-approx-1, echo = FALSE, fig.width=4, fig.height=4}
...
```{r rotation, fig.width=4, fig.height=4, echo=FALSE}
rafalib::mypar()
...
```{r distance-approx-2, echo=FALSE, fig.width=4, fig.height=4}
...
Note that we divided the above by $\sqrt{2}$ to account for the differences in dimensions when comparing a 2 dimenstion distance to a 1 dimension distance. We can actually guarantee that the distance scales remain the same if we re-scale the columns of $A$ to assure that the sum of squares are 1
...
```{r orthogonal-transformation, fig.width=4, fig.height=4, echo=FALSE}
plot(dist(x), dist(z))
abline(0, 1, col = "red")
```
...
Note that by using the transformation above we can summarize the distance between any two pair of twins with just one dimension. For example, one-dimensional data exploration of the first dimension of $Z$ clearly shows that there are two groups, adults and children:
...
In the computation above the total variability in our data can be defined as the sum of the sum of squares of the columns. We assume the column are centered so this sum is equivalent to the sum of the variances of each column
...
The first two dimensions account for 97% of the variability. So we should be able to approximate the distance very well. We can visualize the results of PCA:
