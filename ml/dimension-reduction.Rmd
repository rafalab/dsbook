...
A scatter-plot quickly reveals that the correlation is high and that there are two groups of twins:

```{r simulated_twin_heights, fig.cap="Simulated twin pair heights.",echo=FALSE,message=FALSE}
lim <- c(48,78)
plot(x, xlim=lim, ylim=lim)
```

To help with the illustration, think of this as dataset with two features, the two heights, and $N$ observations. For illustrative purposes, we will act as if visualizing two dimensions is too challenging. We therefore want to reduce the dimensions from two to one, but stil be able to underand important characteristics of the data. For example, that the observations cluster in to two groups: adults and children.

## Preserving distance 

Let's consider a specific challenge: we want a one dimensional summary of our predictors from which we can approximate the distance between any two observations. In the figure below we show the distance between observation 1 and 2, and observation 1 and 51: 1 and 2 are closer:

```{r distance-illustration, echo=FALSE}
plot(x, xlim=lim, ylim=lim)
...
This distance is based on two dimension and we need a distance approximation based on just one.

Let's start with the naive approach of simply removing one of the two dimensions. Let's compare the actual distances to the distance computed with just the first  dimensions. 
...
We obtain a general underestimation. This is actually to be expected because we are adding more positive quantities in the actual distance. If instead we use and average like this:
...
If we look back at the previous scatter plot, and visualize a line between any pair of points, the length of this line is the distance between the two points. These lines tend to go along the direction of the diagonal. Notice that if we instead plot the difference versus the average:

```{r}
z  <- cbind((x[,2] + x[,1])/2,  x[,2] - x[,1])
```

we can see how the distance between points in mostly explained by the first dimenstion: the average.

```{r rotation, fig.cap="Twin height scatterplot (left) and MA-plot (right)."}
...
Dimension reduction can often be described as applying a transformation $A$ to a matrix $X$ with many columns that _moves_ the infromation contained in $X$ to the first few columns of $Z=AX$ then keeping just these few and thus reducing the dimenstion of the vectors contained in the rows.
...
In this particular example to achieve this, we multiply the first set of coefficients (first column of $A$) by $\sqrt{2}$ and the second by $1\sqrt{2}$ then we get the same exact distance if we use both dimensions
...
and a great approximation if we use just the first dimension:
...
Note that by using the transformation above we can summarize the distance between any two pair of twins with just on dimension. For example, one-dimentional data exploration of the first dimension of $Z$ clearly shows the two groups:

```{r }
qplot(z[,1], bins = 20, color = I("black"))
```

We reduced the number of dimensions from two to one with very little loss of information. 
...
However, while the variability in the two columns of `X` is about the same, in the transformed version $Z$ 99% of the variability is included in just the first dimensions:
...
The function PCA returns both the rotation needed to transform $X$ so that the variability of the columns is decreasing from most variable to least as well as the resulting new matrix. By defaul the columns of $X$ are first centered. 

So, using the matrix multiplcition shown above, we have that the following are the same:
...
The rotaion is orhtogonal which means that the inverse is it's transponse. So we also have that these two are identical:
...
We can visualize these to see how the first component summaries the data. In th plot below red represents high values and blue negative values.  Later we learn why we call these weights and patterns):
...
The Iris data is a widely used example. It includes four botanical measurments related to three flower species:

```{r}
names(iris)
```

The data is ordered by the three species:

```{r}
iris$Species
```

Let's compute the distance between each observation. You can clearly see the three species with one species very differnet from the other two:
...
If we apply PCA we should be able to approximate this distance with just two dimensions. Using the `summary` function we can see the variability explained by each PC:
...
The first two dimensions account for 97%. So we should be able to approximate the distance very well. We can visualize the results of PCA:
...
And see that the first pattern is  Sepal Length,  Petal Length and Petal Width (red) in one direction and Sepal Width (blue) in the other. The second patter is the Sepal Length and Petal Width in one direction (blue) and Petal Length and Petal Width in the other (red). You can seee from the weights that the first PC1 drives most the variability and it clearly separates the first third (setosa) from the second two thirds (versicolor and virginica). If you look at the second column of the weights you notice that it somewhat separates versicolor (red) from virgnica (blue).

We can see this better by plotting the first two PCs with color representing the speicies:
...
This example is more realistic than the first since we showed how we can visualize the data, using two dimensions, when the data was four dimensional.
...
And explore the variance of the PCs:
...
The higher PCs appear related to unimportant variability in the corners:
...
Now let's apply the transformation we learned with the trianing data to the test data, reduce the dimenstion and run knn on just a small number of dimensions.
...
With just 36 dimensions  we get an accuraccy well above 0.95.
