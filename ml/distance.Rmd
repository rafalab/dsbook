...
```{r euclidean-distance, echo=FALSE}
library(rafalib)
mypar()
...
The predictor space can be thought of as the collection of all possible vector of predictors that should be considered for the machine learning challenge in question. Each member of the space is referred to as a _point_. So, for example, in the 2 or 7 dataset, the predictor space are all pairs $(x_1, x_2)$ such that both $x_1$ and $x_2$ are within 0 and 1. This particular _space_ can be represented graphically as a square. In the MNIST dataset the predictor space are all 784-th dimensional vectors with each vector element an integer between 0 and 256. An essential element of a predictor space is that we need to define of function that provides the distance between any two points. In most cases we use euclidean distance, but there are other possibilities. A particular case in which we can't simply use euclidean distance is when we have categorical predictors.

Defining a predictor spaces is useful in machine learning because we do things like define neighborhoods of points, as required by many smoothing techniques. For example, we can define a neighborhood as all the points that are within 2 units away from a predefined center. If the points are two dimensional and we use euclidean distance, this neighborhood is graphically represented as a circle with radius 2. In three dimensions as a sphere. We will soon learn about algorithms that partition the space into non-overlapping regions and then makes different prediction for each region using the data in the region. 
