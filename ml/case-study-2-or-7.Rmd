...
Let's try building a machine learning algorithm. We haven't really learned any algorithms yet, so let's start with logistic regression. The model is simply:
...
with $g$ the logistic function: $g(p) = \log\{p / (1-p)\}$. We fit it like this:
...
library(caret)
confusionMatrix(data = y_hat, reference = mnist_27$test$y)[["Accuracy"]]
```
