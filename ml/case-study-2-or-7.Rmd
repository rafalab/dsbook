...
Because we constructed the `mnist_27` example and we had to our disposal 60,000 digits in just the MNIST dataset, we used this to build the _true_ conditional distribution $p(x_1, x_2)$. Note that this is something we don't have access to in practice, but we include it in this example because it lets us compare $\hat{p}(x_1, x_2)$ to the true $p(x_1, x_2)$ and this teaches us the limitations of different algorithms. Let's do that here. We can access and plot $p(x_1,x_2)$ like this:
