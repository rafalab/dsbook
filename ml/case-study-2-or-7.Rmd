...
```{r}
fit <- mnist_27$train %>% mutate(y = ifelse(y==7, 1, 0)) %>% lm(y ~ x_1 + x_2, data = .)
```
...
We get an accuracy of `r confusionMatrix(data = y_hat, reference = mnist_27$test$y)$overall[["Accuracy"]]`. Not bad for our first try. But can we do better?
...
$$
Note that for this boundary, $x_2$ is a linear function of $x_1$. This implies that our logistic regression approach has no chance of capturing the non-linear nature of the true $p(x_1,x_2)$. Here is a visual representation of $\hat{p}(x_1, x_2)$:
...
We can see where the mistakes were made mainly come from low values $x_1$ that have either high or low value of $x_2$. Regression can't catch this.

```{r regression-p-hat-with-points}
p_hat <- predict(fit, newdata = mnist_27$true_p)
mnist_27$true_p %>% mutate(p_hat = p_hat) %>%
  ggplot() +
